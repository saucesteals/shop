...
`expectedDelivery` is optional carrier-provided display text, currently available for UPS. Keep it separate from the latest scan time; do not infer an ETA when absent or present an estimate as guaranteed. Refresh summaries include it when available.
